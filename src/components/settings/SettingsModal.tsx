...
import { X, User, Users, Lock, Mail, Shield, Trash2, CheckCircle2, Loader2 } from 'lucide-react'
...
export function SettingsModal({ session, onClose }: SettingsModalProps) {
  ...
  const fetchData = async () => {
    ...
    if (profData) {
      ...
      // Fetch team members if admin or manager
  ...
  return (
    <div className="modal-overlay" onClick={onClose} style={{ zIndex: 1000 }}>
      <motion.div 
        ...
        style={{ width: '100%', maxWidth: '600px', padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column', maxHeight: '90vh' }}
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 24px', borderBottom: '1px solid var(--glass-border)' }}>
          <h2 style={{ fontSize: '1.25rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '8px' }}>
            <User size={20} color="var(--primary)" /> Settings
          </h2>
          <button onClick={onClose} className="action-btn" style={{ background: 'transparent' }}>
            <X size={20} />
          </button>
        </div>
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid var(--glass-border)' }}>
          <button 
            ...
            style={{ flex: 1, padding: '16px', background: 'transparent', borderBottom: activeTab === 'profile' ? '2px solid var(--primary)' : '2px solid transparent', color: activeTab === 'profile' ? 'var(--primary)' : 'var(--text-muted)', fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', transition: 'all 0.2s' }}
          >
          ...
          {isAdminOrManager && (
            <button 
              ...
              style={{ flex: 1, padding: '16px', background: 'transparent', borderBottom: activeTab === 'team' ? '2px solid var(--primary)' : '2px solid transparent', color: activeTab === 'team' ? 'var(--primary)' : 'var(--text-muted)', fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', transition: 'all 0.2s' }}
            >
          ...
          {loading ? (
            ...
            <form onSubmit={handleUpdateProfile} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div style={{ marginTop: '10px' }}>
                <button type="submit" disabled={updatingProfile} className="primary-gradient action-btn" style={{ width: '100%', height: '44px', justifyContent: 'center' }}>
